...
US_MARKERS = ("美国", "美國", "United States", "USA", "🇺🇸")
...
def pinned_https_connection(hostname, port, address, timeout, context):
    ...
    # HTTPSConnection still verifies TLS against hostname, while the socket is
    # opened only to the public address validated above.
        ...
        try:
            ...
            if content_length:
                try:
                    ...
                    raise SubscriptionUpdateError(
                        "proxy_subscription_response_invalid"
                    ) from error
            ...
            try:
                ...
                raise SubscriptionUpdateError(
                    "proxy_subscription_download_failed"
                ) from error
...
def normalize_provider_payload(raw):
    try:
    ...
    if not isinstance(data, dict):
        raise SubscriptionUpdateError("proxy_subscription_response_invalid")
    proxies = data.get("proxies")
    if not isinstance(proxies, list) or not proxies:
        raise SubscriptionUpdateError("proxy_subscription_proxies_missing")

    folded_markers = tuple(marker.casefold() for marker in US_MARKERS)
    us_proxy_count = 0
    for proxy in proxies:
        name = str(proxy.get("name") or "") if isinstance(proxy, dict) else ""
        folded_name = name.casefold()
        if any(marker in folded_name for marker in folded_markers):
            us_proxy_count += 1
    if us_proxy_count < 1:
        raise SubscriptionUpdateError("proxy_subscription_us_proxy_missing")
...
def read_subscription_url(path=SUBSCRIPTION_PATH):
    try:
    ...
    if len(values) != 1:
        raise SubscriptionUpdateError("proxy_subscription_env_invalid")
    return normalize_subscription_url(values[0])


def atomic_write_provider(
    payload,
    path=PROVIDER_PATH,
    user=PROVIDER_USER,
...
def main():
    ...
    payload = normalize_provider_payload(raw)
    atomic_write_provider(payload)
    print("subscription_update_status=ok")
