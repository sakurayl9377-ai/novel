...
void main() {
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: GameCenterScreen(
          gachaDestinationBuilder: gachaBuilder,
  ...
  testWidgets('shows all entertainment entries with stable keys', (
    tester,
  ) async {
    ...
    try {
      ...
      expect(find.text('樱愿召唤'), findsOneWidget);
      expect(find.text('视觉样片'), findsOneWidget);
      expect(find.text('樱花赛马'), findsOneWidget);
      expect(find.text('百练英雄'), findsOneWidget);
      expect(find.byKey(GameCenterScreen.gachaEntryKey), findsOneWidget);
      ...
      expect(find.bySemanticsLabel('樱愿召唤，视觉样片'), findsOneWidget);
  ...
  testWidgets('uses one column on phones and two columns on tablets', (
    ...
    await pumpCenter(tester, size: const Size(390, 844));
    final phoneGacha = tester.getTopLeft(
      find.byKey(GameCenterScreen.gachaEntryKey),
    );
    final phoneRace = tester.getTopLeft(
      find.byKey(GameCenterScreen.horseRaceEntryKey),
    );
    expect(phoneRace.dx, phoneGacha.dx);
    expect(phoneRace.dy, greaterThan(phoneGacha.dy));

    tester.view.physicalSize = const Size(900, 900);
    await tester.pumpAndSettle();
    final tabletGacha = tester.getTopLeft(
      find.byKey(GameCenterScreen.gachaEntryKey),
    );
    final tabletRace = tester.getTopLeft(
      find.byKey(GameCenterScreen.horseRaceEntryKey),
    );
    expect(tabletRace.dy, tabletGacha.dy);
    expect(tabletRace.dx, greaterThan(tabletGacha.dx));
  });

  testWidgets('gacha entry opens the configured destination', (tester) async {
    await pumpCenter(
      tester,
      size: const Size(390, 844),
      gachaBuilder: (_) => const Scaffold(body: Text('召唤样片页面')),
    );

    await tester.tap(find.byKey(GameCenterScreen.gachaEntryKey));
    await tester.pumpAndSettle();

    expect(find.text('召唤样片页面'), findsOneWidget);
  });
