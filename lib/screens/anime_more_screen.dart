...
class AnimeMoreScreen extends StatefulWidget {
  const AnimeMoreScreen({super.key, required this.title, required this.url});

  final String title;
  final String url;

  @override
  State<AnimeMoreScreen> createState() => _AnimeMoreScreenState();
}

class _AnimeMoreScreenState extends State<AnimeMoreScreen> {
  final AnimeService _service = AnimeService();
  final ScrollController _scrollController = ScrollController();
  final List<AnimeHomeItem> _items = [];
  ...
  void _appendUnique(List<AnimeHomeItem> items) {
    final seen = _items.map((item) => item.id > 0 ? 'id:${item.id}' : item.url);
  ...
  void _openItem(AnimeHomeItem item) {
    if (item.id <= 0) return;
    Navigator.push(
      context,
      MaterialPageRoute(
...
class _MoreAnimeCard extends StatelessWidget {
  const _MoreAnimeCard({
    required this.item,
