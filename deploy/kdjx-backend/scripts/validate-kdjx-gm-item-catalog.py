...
def validate_source(catalog, items_lua):
    if not items_lua.is_file() or items_lua.is_symlink():
    ...
    expected = module.build_catalog(items_lua)
    if catalog != expected:
...
def main():
    ...
    try:
        ...
        validate_source(catalog, Path(args.items_lua).resolve())
