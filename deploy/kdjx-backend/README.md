...
tokens, against those exact sources before it ships the same immutable JSON
used by the admin API. Figure tokens and other ordinary items are rejected
unless their IDs exist in the runtime `items.lua`; client-only configuration
must never extend the delivery allow-list. Every delivery includes the catalog
SHA-256 and is rejected if the admin backend and runtime disagree. The game RPC
also checks the loaded item table before creating a mail. Mail claims validate
attachments before deletion, while bulk claims skip an invalid mail and keep
processing valid ones. The GM bridge is loopback-only and uses an independent
HMAC secret.
